...
{
  function index(Request $request, $type_links, $lang, $siteId, $siteType, $category) {
      if ($type_links == 'pop') {
      ...
      if ($siteType == 'full') {
        $articles = DB::table('sites_articles')
                    ...
                    ->where('articles.category_id', $categoriesArray[$category])
                    //->inRandomOrder()
                    ->orderBy('num_order')
                    ->get()
                    ->toArray();
        ...
        if ($type_links == 'native' || $type_links == 'binom') {
          ...
          $teasersActual = array_slice($teasers, 9, 5);
